...
本手册介绍有关 openSUSE 系统安全的基本内容。其内容覆盖 Linux 上可用的加密机制相关文档，例如 NIS 或者 LDAP。其中也涉及到本地安全相关主题，如访问控制列表，加密和入侵检测。在网络安全部分，您将会了解如何通过防火墙和伪装来进行系统防护，和如何设置虚拟专用网络（VPN）。本手册也将会介绍如何利用产品固有的安全软件，如 AppArmor（用于指定每个程序可以读，写，和执行的文件）。
